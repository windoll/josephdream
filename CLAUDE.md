...
1. **三屬性必須是「槓桿」而非「鏡子」** — 屬性不能只是默默累積、最後分類結局而已。它們要**在過程中實際影響可選項與走向**(門檻、取捨、風險)。任何改動都要維持這點。
   - **機制文法(2026-06 定案,別再堆機制)**:①**每個非結局場景固定恰好 3 個選項**,軸序固定「信心→智慧→野心/結局出口」(改完跑 probe 驗證 3 選項規則);②**全遊戲無隨機性**——「賭一把」機制已整個移除(使用者決定):隨機性與「每個選擇都有後果」的口號及牧養定位衝突(信靠神的 📖 選項擲骰失敗在神學上難圓、團體投影不可控)。原 dothan 逃跑賭注改為智慧≥30 門檻直達 `flee_wild`;③🔒 **所有門檻一律顯示為鎖定按鈕(不可點)**(使用者決定,取代早期的隱藏式門檻),提示「需要X N,目前 M」+**每個鎖必帶 `lockedNote` 敘事短語**,讓鎖是劇情的一部分;④**通往結局的選項由引擎自動加 🏁 結局標籤**(renderChoices 判斷 next 場景有 ending/dynamic),防誤踩一鍵終局。曾因機制太密被玩家反映「故事很亂」,簡化後才定此文法。
...
### 主要場景流向
`start → dothan →`(站住 / 逃跑[智≥30]→`flee_wild` / 記恨)`→`(the_pit 或 flee_wild)`→ memory(父親的故事,主題①,雙路皆經過) → egypt → temptation →`(逃離📖→`prison` / 智取[智≥40]→`outwit→prison` / 屈服🏁→`end_fallen`)`→ prison → pharaoh →`(宣告📖 / 獻策 →`sons`;婉拒🏁→`end_shepherd`)`→ sons(兩個名字,主題③,創41:50–52) → vizier → brothers →`(直接寬恕[信≥50]🏁→`resolve` / 試探📖→`test_brothers` / 報復🏁→`revenge1`)。`test_brothers →`(相認📖🏁→`resolve` / 冷靜確認🏁→`resolve` / 報復🏁→`revenge1`)。`resolve` 為 dynamic 終局。共 18 場景,全程無隨機,每幕恰好三選一。
